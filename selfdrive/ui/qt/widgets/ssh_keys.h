...
class OpkrApksEnableToggle : public ToggleControl {
  Q_OBJECT

public:
  OpkrApksEnableToggle() : ToggleControl("APKs 사용", "어플 사용을 위해 apk환경을 활성화합니다. 활성화시 apk 파일들(소프트키, 맵피, 믹스플로러 등)이 자동 인스톨 됩니다.", "../assets/offroad/icon_shell.png", Params().getBool("OpkrApksEnable")) {
    QObject::connect(this, &OpkrApksEnableToggle::toggleFlipped, [=](int state) {
      bool status = state ? true : false;
